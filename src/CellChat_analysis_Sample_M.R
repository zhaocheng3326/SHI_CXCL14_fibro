...
#' check the Interaction in Sample_H only
rm(list=ls())
...
for (pathways.show in ( netP %>% group_by(pathway_name) %>% summarise(nPair=n_distinct(source,target)) %>% arrange(desc(nPair)) %>% filter(nPair > 10) %>% pull(pathway_name) %>% head(45))) {
  print(pathways.show)
